...
\begin{proof} \leavevmode
    \begin{description}
        \item[(a)] 
        \[
            \widehat{f * g}(\zeta) = \int (f*g)(x) e^{-2 \pi i \zeta x} \; dx = \int \int f(x-y) g(y) \; dy \; e^{-2 \pi i \zeta x} \; dx
        \]  
        Since $f, g \in L^1(\R)$, this integral is finite\footnote{b/c $\leq \| f \|_1 \| g \|_1$ } so 
        by Fubini,
        \[
            = \int \int_{\R} f(x-y) e^{-2 \pi i \zeta (x-y)} \; dx \; g(y) e^{-2 \pi i \zeta y} \; dy
        \] 
        Using $z = x-y$, 
        \[
...
\begin{rem}
    ...
    \[
        ...
        \leq \int \int |f(z)| \; dz \; d \zeta = \infty
    \] 
\end{rem}
...
\begin{proof}[Proof of Thm]
    ...
    Therefore
    \begin{align*}
        \int |(f * K_\epsilon)(x) - f(x)| \; dx &\leq \int \int |f(x-y) - f(x)| K_\epsilon(y) \; dy \; dx \\
        &= \int \int |f(x-y) - f(x) | \frac{1}{\sqrt{\epsilon}} e^{-\pi (y)^2 / \epsilon} \; dy \; dx \\
        &= \int \int |f(x - \sqrt{\epsilon}y) - f(x)| \; dx \; e^{-\pi y^2} \; dy
    \end{align*} 
