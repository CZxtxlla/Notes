...
\begin{defn}[Banach Space]
    $(X, \| . \| )$ is a \newword{Banach space} if $X$ is complete as a metric space,
    with $(p(x, y) = \| x - y \| )$  
\end{defn}
...
\begin{proof}
    ...
    Thus 
    \[
        (x, x_{n_k}) \to (x, x_0) \quad \forall x \in H_0
    \] 
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\leq \iint \left| F(x, y) \right| \; dy \; dx \quad \text{by Minkowski's inequality} \\
    \end{align*}
    ...
    By Tonelli,
    \begin{align*}
        \iint |F(x, y)| \; dy \; dx &= \iint |F(x, y)| \; dx \; dy \\
        &\leq \int |g(y)| \| f \|_1 \; dy \\
        &= \| f \|_1 \| g \|_1 \\
    ...
    \begin{align*}
        ...
        &\stackrel{Holder}{\leq} \left(\int |f(x-y)|^{p/p} |g(y)|^p \; dy\right)^{1/p} \left(\int |f(x-y)|^{q/q} \; dy\right)^{1/q}\\
...
\begin{proof} \leavevmode
    \begin{description}
        ...
        \begin{align*}
            ...
            &\leq \sup_{x \in K} \int_{|y| < \delta} |f(x-y) - f(x)| p_\epsilon(y) \; dy + \sup_{x \in K} \int_{\substack{|y| > \delta \\ |y| \leq \epsilon}} (|f(x-y)| + |f(x)|) p_\epsilon(y) \; dy \\
            &\leq \eta + \sup_{z \in \overline{K + B(0, 1)}} |f(z)| \int_{\substack{|y| > \delta \\ |y| \leq \epsilon}} p_\epsilon(y) \; dy \\
        \end{align*}      
        Therefore 
        \[
